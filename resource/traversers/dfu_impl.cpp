    ...
    resolved_request_t &resolved)
{
    ...
    // (e.g. a node above a slot) is a traversal waypoint locating
    // resources within it and remains valid.
    if (vtx_unit.empty ()) {
        if (req.with.empty ()) {
            m_err_msg += __FUNCTION__;
                               ...
                               share_tally_map_t &tallies)
{
    for (auto &[type, tally] : tallies) {
        ...
        // even though their member edges can be pooled.
        if (tally.shared && tally.per_share > 0 && type != slot_rt) {
            int64_t pooled = dfu.pooled_shares (subsystem, type, tally.per_share);
            if (pooled >= 0) {
                tally.shares = static_cast<unsigned int> (pooled);
                continue;
            }
        }
...
int dfu_impl_t::cnt_slot (const std::vector<Resource> &slot_shape, scoring_api_t &dfu_slot)
{
    ...
    for (auto &slot_elem : slot_shape) {
        ...
        if (is_pooled_elem (dfu_slot, dom, slot_elem)) {
            ...
            // countable here.
            unsigned int per_slot = slot_elem.count.min;
            int64_t pooled = (per_slot > 0) ? dfu_slot.pooled_shares (dom, slot_elem.type, per_slot)
                                            : -1;
            fit = (pooled > 0) ? static_cast<unsigned int> (pooled) : 0;
        } else {
            // Exclusive (or unspecified) resources pack whole egroups per
            // slot: tally disjoint bundles. Non-exclusive unit-less
            // resources (interior waypoints; non-pooled shared leaves are
            // rejected in resolve_request ()) are shareable across slots:
            // tally aggregate capacity (qc / count) instead.
