...
idset_match_writers_t::idset_match_writers_t () : jgf_match_writers_t ()
{
}

idset_match_writers_t::idset_match_writers_t (const idset_match_writers_t &w) : jgf_match_writers_t (w)
{
    m_ids = w.m_ids;
}
...
int idset_match_writers_t::emit_json (json_t **o, json_t **aux)
{
    ...
    // Wrap the array in an object with "idset" key for compatibility
    if (!(obj = json_object ())) {
        json_decref (array);
        rc = -1;
        errno = ENOMEM;
    ...
    if (json_object_set_new (obj, "idset", array) < 0) {
        json_decref (array);
        json_decref (obj);
        rc = -1;
        errno = ENOMEM;
...
const char *idset_match_writers_t::get_uri ()
{
    return m_uri.c_str ();
}
...
////////////////////////////////////////////////////////////////////////////////
// RV1 IDSET Writers Class Method Definitions
////////////////////////////////////////////////////////////////////////////////

jgf_match_writers_t &rv1_idset_match_writers_t::get_jgf ()
{
    return idset_writer;
}
    ...
    try {
        switch (f) {
            ...
            case match_format_t::IDSET:
                w = std::make_shared<rv1_idset_match_writers_t> ();
                break;
