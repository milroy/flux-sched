...
static int test_planner_multi_null_arg_guards ()
{
    ...
    errno = 0;
    ok (planner_multi_new (0, 100, totals, dup_types, 2) == nullptr && errno == EINVAL,
        "planner_multi_new: duplicate resource type");
    errno = 0;
    ok (planner_multi_update (ctx, totals, dup_types, 2) == -1 && errno == EINVAL,
        "planner_multi_update: duplicate resource type");
...
static int test_planner_multi_short_span_vector ()
{
    ...
    // count must fail up front and leave planner state unchanged. A length
    // cannot tell an append from an insertion until spans are keyed by type.
