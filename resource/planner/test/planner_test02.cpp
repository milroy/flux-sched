...
static int test_constuctors_and_overload ()
{
    ...
    *(ctx2->plan_multi) = *(ctx->plan_multi);
    bo = (bo || !(planner_multis_equal (ctx, ctx2)));
    ...
    *(ctx4->plan_multi) = *(ctx2->plan_multi);
    size = planner_multi_span_size (ctx4);
    ok ((size == 2), "planner_multi 3 now has the size of planner_multi 2");
    bo = (bo || !(planner_multis_equal (ctx2, ctx4)));
    // Copy constructor works on planners with updated state
    planner_multi_destroy (&ctx);
    ...
    ok (!bo, "planner_multi constructors and overload work");
