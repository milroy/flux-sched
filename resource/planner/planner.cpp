...
planner::planner ()
{
    ...
    m_plan_end = 0;
    //m_p0 = nullptr;
    m_p0 = std::make_shared <scheduled_point_t> ();
    m_p0->at = 0;
    m_p0->ref_count = 1;
    m_p0->remaining = 0;
...
planner::planner (const planner &o)
{
    ...
    m_current_request = o.get_current_request ();
...
int planner::mt_tree_insert (std::shared_ptr <scheduled_point_t> point)
{
    return m_mt_resource_tree.insert (point);
}

int planner::mt_tree_remove (std::shared_ptr <scheduled_point_t> point)
{
    return m_mt_resource_tree.remove (point);
}

int planner::sp_tree_insert (std::shared_ptr <scheduled_point_t> point)
{
    return m_sched_point_tree.insert (point);
}

int planner::sp_tree_remove (std::shared_ptr <scheduled_point_t> point)
{
...
std::shared_ptr<scheduled_point_t> planner::sp_tree_search (int64_t at)
{
    return m_sched_point_tree.search (at);
}

std::shared_ptr<scheduled_point_t> planner::sp_tree_get_state (int64_t at)
{
    return m_sched_point_tree.get_state (at);
}

std::shared_ptr<scheduled_point_t> planner::sp_tree_next (std::shared_ptr<scheduled_point_t> point)
{
    return m_sched_point_tree.next (point);
}

std::shared_ptr<scheduled_point_t> planner::sp_tree_next (std::shared_ptr<scheduled_point_t> point) const
{
    return m_sched_point_tree.next (point);
}

std::shared_ptr<scheduled_point_t> planner::mt_tree_get_mintime (const int64_t request) const
{
...
const std::map<int64_t, std::shared_ptr<span_t>> &planner::get_span_lookup () const
{
...
std::map<int64_t, std::shared_ptr<span_t>>::const_iterator planner::get_span_lookup_iter () const
{
    return m_span_lookup_iter;
}

std::map<int64_t, std::shared_ptr<span_t>>::iterator planner::get_span_lookup_iter ()
{
    return m_span_lookup_iter;
}
...
const std::shared_ptr <scheduled_point_t> &planner::get_p0 () const
{
    return m_p0;
}
...
const std::map<int64_t, std::shared_ptr<scheduled_point_t>> &planner::get_avail_time_iter () const
{
    return m_avail_time_iter;
}

const request_t &planner::get_current_request () const
{
...
int planner::copy_trees (const planner &o)
{
    ...
    std::shared_ptr<scheduled_point_t> point = m_p0;
    std::shared_ptr<scheduled_point_t> new_point = nullptr;
    while (point) {
        new_point = std::make_shared<scheduled_point_t> ();
        new_point->at = point->at;
        new_point->in_mt_resource_tree = 0;
...
int planner::copy_maps (const planner &o)
{
    ...
    std::shared_ptr<span_t> new_span = nullptr;
    for (auto &span_it : o.get_span_lookup ()) {
        std::shared_ptr<span_t> new_span = std::make_shared<span_t> ();
        *new_span = *(span_it.second);
        m_span_lookup.emplace (span_it.first, new_span);
    }
    std::shared_ptr<scheduled_point_t> new_avail = nullptr;
    for (auto &avail_it : o.get_avail_time_iter ()) {
        new_avail = std::make_shared<scheduled_point_t> ();
...
int planner::clear ()
{
    ...
    //   std::cout << "p0 use count: " << m_p0.use_count () << "\n";
    //}
    m_sched_point_tree.clear ();
...
static int track_points (std::map<int64_t, std::shared_ptr <scheduled_point_t> > &tracker,
                         std::shared_ptr <scheduled_point_t> &point)
{
...
{
    std::shared_ptr<scheduled_point_t> point = nullptr;
    for (auto &kv : ctx->plan->get_avail_time_iter ())
        ctx->plan->mt_tree_insert (kv.second);
    ctx->plan->clear_avail_time_iter ();
}

static void update_mintime_resource_tree (planner_t *ctx,
                                          std::list<std::shared_ptr <scheduled_point_t> > &list)
{
    std::shared_ptr <scheduled_point_t> point = nullptr;
    for (auto &point : list) {
...
static std::shared_ptr <scheduled_point_t> get_or_new_point (planner_t *ctx, int64_t at)
{
    std::shared_ptr <scheduled_point_t> point = nullptr;
    try {
        if ( !(point = ctx->plan->sp_tree_search (at))) {
            std::shared_ptr <scheduled_point_t> state = ctx->plan->sp_tree_get_state (at);
            point = std::make_shared<scheduled_point_t> ();
            point->at = at;
...
static void fetch_overlap_points (planner_t *ctx, int64_t at, uint64_t duration,
                                  std::list<std::shared_ptr <scheduled_point_t> > &list)
{
    std::shared_ptr <scheduled_point_t> point = ctx->plan->sp_tree_get_state (at);
    while (point) {
...
static bool span_ok (planner_t *ctx, std::shared_ptr <scheduled_point_t> start_point,
...
{
    int64_t at = -1;
    std::shared_ptr <scheduled_point_t> start_point = nullptr;
...
static std::shared_ptr <scheduled_point_t> avail_resources_during (planner_t *ctx, int64_t at,
                                                  uint64_t duration)
{
    if (static_cast<int64_t> (at + duration) > ctx->plan->get_plan_end ()) {
        errno = ERANGE;
    ...
    std::shared_ptr <scheduled_point_t> point = ctx->plan->sp_tree_get_state (at);
    std::shared_ptr <scheduled_point_t> min = point;
    while (point) {
        if (point->at >= (at + (int64_t)duration))
            break;
...
{
    ctx->plan->clear_span_lookup ();
    ctx->plan->clear_avail_time_iter ();
    if (ctx->plan->get_p0 () && ctx->plan->get_p0 ()->in_mt_resource_tree)
        ctx->plan->mt_tree_remove (ctx->plan->get_p0 ());
    ctx->plan->destroy_sp_tree ();
}
                              ...
                              int64_t base_time, uint64_t duration)
{
    ...
    uint64_t resource_totals = ctx->plan->get_total_resources ();
    const char *resource_type = ctx->plan->get_resource_type ().c_str ();
...
extern "C" int64_t planner_avail_time_next (planner_t *ctx)
{
    ...
    request_count = ctx->plan->get_current_request ().count;
    on_or_after = ctx->plan->get_current_request ().on_or_after;
    duration = ctx->plan->get_current_request ().duration;
...
{
    std::shared_ptr <scheduled_point_t> min_point = nullptr;
...
{
    std::shared_ptr <scheduled_point_t> state = nullptr;
                                     ...
                                     uint64_t duration, uint64_t request)
{
    std::shared_ptr<span_t> span = nullptr;
    std::shared_ptr <scheduled_point_t> start_point = nullptr;
    std::shared_ptr <scheduled_point_t> last_point = nullptr;
