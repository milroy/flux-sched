...
public:
    ...
    scheduled_point_t *sp_tree_search (int64_t at) const;
    scheduled_point_t *sp_tree_get_state (int64_t at) const;
    const scheduled_point_t *sp_tree_next (const scheduled_point_t *point) const;
    scheduled_point_t *mt_tree_get_mintime (int64_t request) const;
    void clear_avail_time_iter ();
    void clear_span_lookup ();
    std::map<int64_t, std::shared_ptr<span_t>>::iterator &span_lookup_erase (std::map<int64_t, std::shared_ptr<span_t>>::iterator &it);
