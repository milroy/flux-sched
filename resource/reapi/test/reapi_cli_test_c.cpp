...
{
    // 0 satisfiable, 1 unsatisfiable, -1 error.  The match itself returns -1
    // for both an unsatisfiable request (errno=ENODEV) and an error, and the
    // wrapper tells them apart by errno alone, so an allocator that clobbered
    // errno inside free (R) would misclassify the verdict.
    const std::string options = "{}";
    std::stringstream gbuffer, jbuffer;
    const char *test_srcdir = std::getenv ("SHARNESS_TEST_SRCDIR");
    REQUIRE (test_srcdir);
