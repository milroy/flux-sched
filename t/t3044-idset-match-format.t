...
test_expect_success 'idset emits complete R with idset scheduling format' '
    ...
    jq -e "type == \"object\"" match1.json &&
    jq -e ".version == 1" match1.json &&
    jq -e ".execution" match1.json &&
    jq -e ".scheduling.idset | type == \"array\"" match1.json &&
    jq -e ".scheduling.idset | length > 0" match1.json
'

test_expect_success 'idset scheduling contains only string vertex IDs' '
    jq -e ".scheduling.idset | all(.[]; type == \"string\")" match1.json
'
...
test_expect_success 'idset emits all vertex IDs with policy=low' '
    ...
    jq -e "type == \"object\"" match2.json &&
    jq -e ".scheduling.idset | type == \"array\"" match2.json &&
    jq -e ".scheduling.idset | length > 0" match2.json
'

test_expect_success 'idset output with policy=low has more IDs than lonodex' '
    match1_count=$(jq ".scheduling.idset | length" match1.json) &&
    match2_count=$(jq ".scheduling.idset | length" match2.json) &&
...
EOF
    ...
    jq -e "type == \"object\"" match3.json &&
    jq -e ".scheduling.idset | type == \"array\"" match3.json &&
    jq -e ".scheduling.idset | length > 0" match3.json &&
    match3_count=$(jq ".scheduling.idset | length" match3.json) &&
