...
test_expect_success 'generate idset R to use for updates' '
    jq -S .scheduling R_JGF.json > JGF.json &&
    ${query} -L JGF.json -f jgf -F idset -t idset1.out -P lonodex < match_jobspec_cmd &&
    head -n1 idset1.out | jq -S . > R_idset.json &&
    jq -e "type == \"object\"" R_idset.json &&
    jq -e ".version == 1" R_idset.json &&
    jq -e ".scheduling.idset | type == \"array\"" R_idset.json &&
    jq -e ".scheduling.idset | length > 0" R_idset.json &&
    jq -S .scheduling R_idset.json > idset.json
'

test_expect_success 'idset scheduling contains only vertex ID strings' '
    jq -e ".idset | all(.[]; type == \"string\")" idset.json
'
